...
public class OrderEventServiceTests {
    ...
    void init() {
        ...
        rName = "測試餐廳";
        mockCreateTime =
            Timestamp.valueOf(LocalDateTime.of(2023, 11, 24, 12, 30, 0));
        mockStopOrderingTime =
            Timestamp.valueOf(LocalDateTime.of(2023, 11, 24, 14, 30, 0));
        mockEndEventTime =
            Timestamp.valueOf(LocalDateTime.of(2023, 11, 25, 12, 30, 0));
        mockEstimatedArrivalTime =
            Timestamp.valueOf(LocalDateTime.of(2023, 11, 24, 15, 00, 0));
    }

    OrderEventModel constructOrderEvent(
        int rid,
        String rName,
        int hostId,
        Timestamp createTime,
        Timestamp stopOrderingTime,
        Timestamp endEventTime,
    ...
    void createOrderEvent() {
        OrderEventModel orderEvent = constructOrderEvent(
            1,
            null,
            2,
            mockCreateTime,
        ...
        Mockito
            .when(orderEventRepository.createOrderEvent(orderEvent))
            .thenReturn(1);

        Assertions.assertEquals(
            orderEventService.createOrderEvent(orderEvent),
            1
        );
    }
    ...
    void getOrderEventByOid() {
        ...
        Mockito
            .when(orderEventRepository.getOrderEventByOid(oid))
            .thenReturn(orderEvent);

        Assertions.assertEquals(
            orderEventService.getOrderEventByOid(oid),
            orderEvent
        );
    }
}
