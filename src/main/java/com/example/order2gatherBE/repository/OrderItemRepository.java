...
public class OrderItemRepository {
    ...
    public void initOrderEventMembers(int uid, int oid) {
        ...
        jdbcTemplate.update(
            "INSERT INTO userOrderFood(uid, oid, foodName, hostViewFoodName, price, hostViewPrice, num, comment, fid) " +
            "VALUES (?,?,?,?,?,?,?,?,?)",
            uid,
            oid,
            "",
    ...
    public void addOrderItem(OrderItemModel item) {
        ...
        jdbcTemplate.update(
            ...
            item.getOID(),
            item.getFoodName(),
            item.getHostViewFoodName(),
            item.getPrice(),
            item.getHostViewPrice(),
            item.getNum(),
            item.getComment(),
            item.getFID()
        );
    ...
    public void modifyOrderItem(OrderItemModel item) {
        ...
        if (hid == item.getUID()) {
            ...
            jdbcTemplate.update(
                "UPDATE userOrderFood SET foodName=?, hostViewFoodName=?, price=?, hostViewPrice=?, num=?, comment=? " +
            ...
            jdbcTemplate.update(
                "UPDATE userOrderFood SET foodName=?, price=?, num=?, comment=? " +
                "WHERE uid=? and oid=? and fid=?",
                item.getFoodName(),
                item.getPrice(),
                item.getNum(),
    ...
    public void deleteOrderItem(OrderItemModel item) {
        ...
        jdbcTemplate.update(
            "DELETE FROM userOrderFood " + "WHERE uid=? and oid=? and fid=?",
            item.getUID(),
            item.getOID(),
            item.getFID()
        );
    ...
    public int getHost(int oid) {
        ...
        List<OrderEventModel> temp = jdbcTemplate.query(
            sql_1,
            new BeanPropertyRowMapper<>(OrderEventModel.class),
            oid
        );
    ...
    public List<Integer> getUsers(int oid) {
        ...
        List orders = jdbcTemplate.queryForList(
            "SELECT * FROM userOrderFood WHERE oid=" + oid + " and fid=-1"
        );
    ...
    public List<OrderItemModel> getUserOrderItem(int uid, int oid) {
        ...
        List orders = jdbcTemplate.queryForList(
            "SELECT * FROM userOrderFood WHERE oid=" + oid + " and uid=" + uid
        );
    ...
    public List<OrderItemModel> getAllOrderItem(int oid) {
        System.out.println("EXCUTE getAllItem");
        List orders = jdbcTemplate.queryForList(
            "SELECT * FROM userOrderFood WHERE oid=" + oid
        );
        List<OrderItemModel> items = new ArrayList<OrderItemModel>();
    ...
    public List<OrderItemModel> getHostViewOrderItems(int oid) {
        String sql =
            "SELECT uof.fid, uof.uid, uof.foodName, uof.hostViewFoodName, uof.price, uof.hostViewPrice, uof.num, uof.comment, user.username " +
