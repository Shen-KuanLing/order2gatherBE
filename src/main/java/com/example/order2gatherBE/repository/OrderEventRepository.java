...
public class OrderEventRepository {
    ...
    public int createOrderEvent(OrderEventModel orderEventModel) {
        ...
        int createStatus = jdbcTemplate.update(
                connection -> {
                    ...
                    ps.setTimestamp(
                        6,
                        orderEventModel.getEstimatedArrivalTime()
                    );
        ...
        int addMemberStatus = addMemberList(
            orderEventModel.getMemberList(),
            orderEventId
        );
    ...
    public int addMemberList(List<Integer> memberList, Integer oid) {
        try {
            String insertQuery =
                "INSERT INTO userOrderFood (uid, oid, hostViewFoodName, fid) VALUES (?,?,?,?)";
    ...
    public OrderEventModel getOrderEventByOid(Integer oid) {
        String sql =
            "SELECT oe.id, oe.rid, oe.hostId, oe.secretcode, oe.createTime, oe.stopOrderingTime, " +
    ...
    public List<OrderEventModel> getOrderEventByUid(Integer uid) {
        String sql =
            "SELECT oe.*, (SELECT r.name FROM restaurant r WHERE r.id = oe.rid) AS restaurantName, u.gmail AS hostGmail " +
            "FROM orderEvent oe " +
    ...
    public void updateOrderEvent(OrderEventModel updatedOrderEvent) {
        String sql =
            "UPDATE orderEvent SET stopOrderingTime = ?, estimatedArrivalTime = ?, endEventTime = ?, " +
            "status = ? WHERE id = ?";
    ...
    public boolean isUserInOrderEvent(Integer uid, Integer oid) {
        String sql =
            "SELECT COUNT(*) FROM userOrderFood WHERE uid = ? AND oid = ?";
        return (
    ...
    public void updateOrderEventTotalPeople(Integer oid) {
        String sql =
            "UPDATE orderEvent SET totalPeople = totalPeople + 1 WHERE id = ?";
