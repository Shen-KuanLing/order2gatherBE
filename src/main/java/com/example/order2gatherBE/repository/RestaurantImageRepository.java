...
public class RestaurantImageRepository {
    ...
    public List<RestaurantImageModel> get(int rid) {
        ...
        try {
            images =
                (List<RestaurantImageModel>) this.jdbcTemplate.query(
                        sql,
                        new BeanPropertyRowMapper<RestaurantImageModel>(
                            RestaurantImageModel.class
                        ),
                        rid
                    );
        } catch (Exception e) {
            throw new DataAccessException(
                500,
                "[SQL EXCEPTION]: Fail to get menu image",
                e.getMessage()
            );
        }
        return images;
    ...
    public String save(RestaurantImageModel menu) {
        String sql =
            "Insert INTO restaurantImage(rid, menuImage) VALUES (?, ?)  ";
        try {
            ...
            throw new DataAccessException(
                500,
                "[SQL EXCEPTION]: Fail to save Images",
                e.getMessage()
            );
        }
    }
