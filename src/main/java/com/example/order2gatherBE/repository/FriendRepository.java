...
public class FriendRepository {
    ...
    public boolean add(int uid1, int uid2, String user2NickName) {
        ...
        String sql = "INSERT INTO `friend` (`user1`, `user2`, `user2NickName`) VALUES (?, ?, ?);";
        if (user2Name == null) {
            user2Name = authenticationRepository.findUserbyId(uid2).getUsername();
        }
    ...
    public boolean isFriend(int uid1, int uid2) {
        String sql = "select count(*) from friend where user1 = ? and user2 = ?";
    ...
    public int createGroup(String gname) {
        ...
        if (jdbcTemplate.update(sql, gname) > 0) {
            int res = jdbcTemplate.queryForObject("SELECT @@IDENTITY as id",
                    Integer.class);
            return res;
        }
        return -1;
    ...
    public List<Map<String, Object>> getUserFriend(int uid) {
        String sql = "Select user.id as id, user.username as username, user.gmail as email, " +
    ...
    public List<Map<String, Object>> getGroupMember(int gid) {
        ...
        List<Map<String, Object>> groupMember = jdbcTemplate.queryForList(sql, gid);
        return groupMember;
