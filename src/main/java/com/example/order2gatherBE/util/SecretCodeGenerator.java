...
public class SecretCodeGenerator {
    public static String generateSecretCode(int hostID) {
        try {
            ...
            String inputString = String.valueOf(currentTimeMillis) + String.valueOf(hostID);
            ...
            System.arraycopy(hashedBytes, 0, truncatedHash, 0, truncatedHash.length);

            // Convert the truncated hash to a 6-digit string
            int secretCode = Math.abs(ByteBuffer.wrap(truncatedHash).getInt() % 1000000);
