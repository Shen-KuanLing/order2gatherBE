...
public class RestaurantController {
    ...
    public ResponseEntity<?> getRestaurants(
        ...
        if (formatMap.isEmpty()) {
            ...
            displayResponse.setMessage(message);
            if (formatMap.containsKey("menu")) displayResponse.setMenu(
                (List<String>) formatMap.get("menu")
            );

            if (formatMap.containsKey("food")) displayResponse.setFood(
                (List<FoodModel>) formatMap.get("food")
            );
            if (
                formatMap.containsKey("restaurant")
            ) displayResponse.setRestaurant(
                (List<RestaurantModel>) formatMap.get("restaurant")
            );
        }

        return ResponseEntity.status(code).body(displayResponse);
    }
    ...
    public ResponseEntity<?> deleteRestaurantDetail(
        ...
        //Generate Response
        if (success == 0) response.put(
            "message",
            "Please refrain from deleting information about others' restaurants"
        ); else response.put(
            "message",
            String.format("The %s Restaurant has been deleted.", rid)
        );
    ...
    public ResponseEntity<?> deleteFood(
        ...
        if (success == 0) response.put(
            "message",
            "Please refrain from deleting information about others' foods."
        ); else response.put(
            "message",
            String.format("The %s food has been deleted.", rid)
        );
    ...
    public ResponseEntity<?> putRestaurantDetail(
        ...
        if (rest != null) {
            ...
            if (updateCounts != 0) {
                ...
                message =
                    "Fail to update restaurant and image and food, because the restaurant is deleted or didn't exist! ";
            }
        }
        HttpStatus code = HttpStatus.OK;
        if (foods != null) {
