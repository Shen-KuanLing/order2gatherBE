...
public class OrderItemController {
    ...
    public ResponseEntity<String> addUserOrderItem(
        @RequestHeader(HttpHeaders.AUTHORIZATION) String token,
        @RequestBody OrderItemModel formData
    ) {
        token = token.replace("Bearer ", "");
        int auth_uid = authenticationService.verify(token);
        if (auth_uid == -1) {
        ...
        return new ResponseEntity<>(
            ("food :" + formData.getFoodName() + " added"),
            HttpStatus.OK
        );
    }
    ...
    public ResponseEntity<List<Integer>> getUsers(
        @RequestHeader(HttpHeaders.AUTHORIZATION) String token,
    ...
    public ResponseEntity<List<OrderItemModel>> getUserOrderItem(
        @RequestHeader(HttpHeaders.AUTHORIZATION) String token,
        ...
        return new ResponseEntity<>(
            orderItemService.getUserOrderItem(uid, oid),
            HttpStatus.OK
        );
    }
    ...
    public ResponseEntity<List<OrderItemModel>> getAllOrderItem(
        @RequestHeader(HttpHeaders.AUTHORIZATION) String token,
        @RequestParam int oid
    ) {
        token = token.replace("Bearer ", "");
        ...
        return new ResponseEntity<>(
            orderItemService.getAllOrderItem(oid),
            HttpStatus.OK
        );
    }
    ...
    public ResponseEntity<String> modifyUserOderItem(
        @RequestHeader(HttpHeaders.AUTHORIZATION) String token,
        @RequestBody OrderItemModel formData
    ) {
        token = token.replace("Bearer ", "");
        ...
        if (hid == formData.getUID()) return new ResponseEntity<>(
            ("Host: fid-" + formData.getFID() + " modified"),
            HttpStatus.OK
        ); else return new ResponseEntity<>(
            ("Orderer: fid-" + formData.getFID() + " modified"),
            HttpStatus.OK
        );
    }
    ...
    public ResponseEntity<String> deleteUserOderItem(
        @RequestHeader(HttpHeaders.AUTHORIZATION) String token,
        @RequestBody OrderItemModel formData
    ) {
        token = token.replace("Bearer ", "");
        ...
        return new ResponseEntity<>(
            ("food :" + formData.getFoodName() + " delete"),
            HttpStatus.OK
        );
    }
}
